--[[
 ...
 UI moderna + ms funciones + mejor rendimiento
 By Zvolt
]]
...
-- (Si no, cada execute duplica ESP/aim/fly y pelean entre s.)
...
--// GUI raz (nombres neutros: los anticheats escanean instancias con "hub", "aim", "esp", "zvolt", etc.)
...
-- STEALTH: siempre PlayerGui. gethui/CoreGui es una bandera clsica para los anticheats.
...
local function notify(title, text)
 ...
 -- NOTA: no se usa StarterGui:SetCore a propsito (rompe mdulos CoreGui en algunos executors)
end
...
-- Cambia palabras sensibles por neutras. Prendido desde la construccin (si patean
-- al ejecutar, un interruptor interno no servira). "Nombres reales" lo revierte.
...
local DMDICT = {
 ...
 {"Municin infinita", "Municion"}, {"MUNICIN INFINITA", "MUNICION"},
...
-- Estado del aimbot en pantalla (diagnstico en vivo, sin F9)
...
do
 ...
 closeBtn.MouseButton1Click:Connect(function() mainFrame.Visible = false notify("Zvolt", "UI oculta. Pulsa el botn ZVOLT o RightShift.") end)
...
local pages, tabBtns = {}, {}
local TAB_INFO = {
 {id="combat", icon="", title="Combat", desc="Apunta, pega y domina."},
 {id="visuals", icon="", title="Visuals", desc="Ve todo antes que ellos."},
 {id="movement", icon="", title="Movement", desc="Muvete sin lmites."},
 {id="teleport", icon="", title="Teleport", desc="Viaja instantneo."},
 ...
 {id="config", icon="", title="Config", desc="Teclas, tema y cuenta."},
}
...
local function createCard(page, titleText, height)
 ...
 -- altura mnima para que no se vea vaca (el height que se pasaba antes)
...
local function createSlider(parent, titlePrefix, defaultVal, minVal, maxVal, callback)
 ...
 -- FIX tctil: usar i.Position (el dedo), no GetMouseLocation (no se actualiza en mvil y el slider saltaba)
...
--// MODO FANTASMA: solo permite lo que el servidor NO puede ver (ESP, aimbot de cmara, luz).
-- Los anticheats detectan: WalkSpeed, tamao de hitbox, teleports, hooks, fly, noclip.
local riskyToggles = {}
...
--// PERFILES SILENT POR JUEGO (estilo silent.lua del video: remote + ndices exactos).
-- Universal = escaneo automtico. Los dems se llenan con la data del F9 que pases.
-- originIdx/dirIdx/hitIdx = posicin del argumento (1 = primero). hitIdx es opcional.
...
local tSilent
tSilent = createToggle(c4, "Silent Aim hook detectable", function(v)
...
createButton(c4, " Copiar datos del juego", function()
 ...
 setclipboard(s)
 notify("Silent", "Datos copiados. Psamelos con el log del SPY.")
 else
...
local tMagic
tMagic = createToggle(c4, "Magic Bullets ", function(v)
 ...
 notify("Magic Bullets", "Activadas + Silent auto (usan su FOV y prediccin)")
 else
...
createToggle(c1, "Diagnstico F9", function(v) settings.aimDebug = v end)
...
createToggle(c2, "Mostrar crculo FOV", function(v) settings.showFov = v end, nil, true)
createToggle(c2, "FOV arcoris ", function(v) settings.fovRainbow = v end)

local c3 = createCard(pages["combat"], " Dao & Paredes", 150)
local tHitbox
tHitbox = createToggle(c3, "Hitbox Extender MUY detectable", function(v)
...
createSlider(c3, "Tamao Hitbox", settings.hitboxSize, 2, 25, function(v) settings.hitboxSize = v end)
local tWallbang
tWallbang = createToggle(c3, "Wallbang + X-ray ", function(v)
...
-- VISUALS
local v1 = createCard(pages["visuals"], " ESP Principal", 280)
...
createToggle(v1, "Tracers / Lneas", function(v) settings.espTracer = v end)
...
createDropdown(v2, "Origen de lneas", {"Bottom", "Center", "Mouse"}, "Bottom", function(v) settings.tracerOrigin = v end)
createToggle(v2, "ESP arcoris ", function(v) settings.espRainbow = v end)
createSlider(v2, "Distancia mxima", settings.maxDistance, 100, 5000, function(v) settings.maxDistance = v end)

-- MOVEMENT
local m1 = createCard(pages["movement"], " Vuelo & Noclip", 150)
local tFly
...
local tSpeed
tSpeed = createToggle(m2, "Custom Speed detectable", function(v)
...
local tInfJump
tInfJump = createToggle(m2, "Salto infinito ", function(v)
...
local tSpin
tSpin = createToggle(m3, "Spinbot detectable", function(v)
...
local tJerk
tJerk = createToggle(m3, "Jerk anti-aim ", function(v)
...
local tCtrl
tCtrl = createToggle(m3, "Ctrl + Click TP ", function(v)
...
local tTap
tTap = createToggle(m3, "Tap TP tctil", function(v)
...
createButton(t2, " Guardar mi posicin", function()
 local r = myRoot() if r then settings.savedPos = r.CFrame notify("TP", "Posicin guardada.") end
end, false)
createButton(t2, " Volver a posicin guardada", function()
 if ghostBlock() then return end
...
local tOrbit
tOrbit = createToggle(tr1, "rbita", function(v)
...
createSlider(tr1, "Velocidad rbita", settings.orbitSpeed, 1, 15, function(v) settings.orbitSpeed = v end)
createSlider(tr1, "Radio rbita", settings.orbitRadius, 3, 20, function(v) settings.orbitRadius = v end)
...
local tAmmo
tAmmo = createToggle(w1, "Municin infinita ", function(v)
...
createButton(w1, " Reset cmara", function()
 camera.FieldOfView = 70 notify("Weapon", "Cmara reseteada.")
end, false)
...
-- WORLD
local wo1 = createCard(pages["world"], " Iluminacin & Mapa", 190)
...
createButton(wo1, " Poner de da", function()
...
createButton(wo1, " Quitar niebla", function()
 Lighting.FogEnd = 100000 for _, v in ipairs(Lighting:GetChildren()) do if v:IsA("Atmosphere") then v.Density = 0 end end
end, false)
createSlider(wo1, "Hora del da", 14, 0, 24, function(v) Lighting.ClockTime = v end)
...
createButton(wo2, " FPS BOOST", function()
 if settings.fpsBoosted then notify("World", "Ya est aplicado.") return end
...
createToggle(cfGhost, "Log F9 (solo diagnstico)", function(v)
 settings.verbose = v
 if v then
 notify("Consola", "Log activado: salo y apgalo, el anticheat lee la consola.")
...
createToggle(cf1, "ESP arcoris directo", function(v) settings.espRainbow = v end)
...
local cf2 = createCard(pages["config"], " Keybinds", 330)
...
createButton(cf3, " Destruir Zvolt", function()
...
-- Botones tctiles para subir/bajar volando
...
-- El character se trackea aparte en espCharOf, que clearESP jams destruye.
local espData = {}
local espCharOf = {}
...
local function clearESP(plr)
 ...
 -- no tocamos espCharOf aqu a propsito en el loop por frame;
...
-- Punto de referencia del aim: cursor en PC, centro de pantalla en tctil (no hay cursor)
...
--// SILENT AIM (redirige tiros sin mover la cmara)
...
local silentRedirects = 0 -- telemetra: tiros redirigidos en total
local silentLastTarget = nil -- ltimo objetivo adquirido
...
local function getSilentHitPos()
 ...
 -- bots: tambin para el silent (si tus enemigos no son Players, aqu los encuentra)
...
-- Detecta piezas que NACEN cerca tuyo, en movimiento y alejndose (tus balas), e ignora el resto.
local trackedBullets = {}
local magicAcquired = 0 -- telemetra: balas con objetivo asignado
workspace.DescendantAdded:Connect(function(inst)
 ...
 if off.Magnitude < 1 or off.Magnitude > 25 then return end -- naci pegado o lejos: no es tu bala
 local vel = inst.AssemblyLinearVelocity
 if vel.Magnitude < 20 then return end -- quieto: decoracin del mapa, no bala
...
RunService.Heartbeat:Connect(function()
 ...
 if sp > 5 then
 -- conserva la velocidad original: SOLO dobla la direccin (natural y menos detectable)
...
--// SPY: registra qu manda tu arma (remotes/raycasts) en la consola F9. Solo LEE, indetectable.
-- salo una vez por juego: dispara varias veces, abre F9 y pasa el log para adaptar el silent exacto.
...
workspace.DescendantAdded:Connect(function(inst)
 ...
 dprint("[ZVOLT-SPY] parte rpida cerca:", pn, "vel:", math.floor(inst.AssemblyLinearVelocity.Magnitude))
end)
function tryEnableSilentAim()
 ...
 local nc = (newcclosure and newcclosure) or function(f) return f end
 if not (hm and gncm) then error("executor sin hookmetamethod") end
 -- Hook nico __namecall: 1) raycasts del cliente 2) remotes con hitreg en servidor
 ...
 local hitPos = getSilentHitPos()
 if hitPos then
 -- 0) perfil del juego: reemplazo quirrgico si conocemos su remote (video style)
 do
 ...
 for i = 1, n do nargs[i] = args[i] end
 local changed = false
 -- 1) pares (origen, direccin): el formato ms comn (el server hace el raycast).
 ...
 -- 2) Vector3 de POSICIN cerca de tu mira pero LEJOS de ti (nunca el origen).
 for i = 1, n do
 local a = nargs[i]
 ...
 notify("Silent Aim ", "Tu executor no soporta hooks (" .. tostring(msg) .. "). Usa Aimbot normal.")
 end
end

-- El aimbot V1 corre en el loop principal abajo (RenderStepped plano, como el original).

-- Re-aplicacin post-cmara: el juego reescribe su cmara cada frame; con esto
-- la ltima escritura siempre es la nuestra (Render + post-cmara + Heartbeat).
prevCamType = nil
...
pcall(function() RunService:UnbindFromRenderStep("ZV_Aimbot") end)
pcall(function()
 -- prioridad Last (2000): corre DESPUS de cualquier cmara del juego, justo antes del render
...
local aimDeepLast = 0
local lastAimPart = nil -- ltimo objetivo: se re-aplica post-cmara aunque el juego la pise
local espFrame, noclipLast, fbLastRefresh = 0, 0, 0 -- acumuladores de rendimiento
RunService.RenderStepped:Connect(function(dt)
 if dead then return end
 -- La cmara puede ser REEMPLAZADA por el juego (rondas/respawns). Si usamos la vieja,
 -- todo da mal (pant:0) y escribimos en una cmara invisible. Re-leer cada frame lo arregla.
 local freshCam = workspace.CurrentCamera
 if freshCam then camera = freshCam end
 if not camera then return end
 -- red de seguridad: cualquier error aqu saldra a consola con "loadstring" (baneable)
 ...
 if wantFov then
 -- cada aim usa su propio radio: el crculo muestra el del aimbot, o el del silent si solo ese est activo
 ...
 -- el crculo se engrosa un instante cuando el silent redirige un tiro (confirmacin visual)
 ...
 -- Aimbot estilo V1: RenderStepped plano, sin filtros extra. Frmula del original que s funciona.
 -- El estado muestra el embudo (vivos -> con parte -> en pantalla) para ver qu filtra a todos.
 do
 local trigV1 = isAimingRightClick or aimingMobile or settings.aimAuto
 if not settings.aimEnabled then
 aimStatus.Text = ""
 elseif not trigV1 then
 aimStatus.Text = discreetText("AIM: ON - mantn click derecho")
 ...
 -- zona muerta: si ya ests encima del objetivo no toca la cmara (tu mouse manda)
 ...
 -- Telemetra silent/magic en pantalla + F9 (para diagnosticar qu no redirige)
 do
 ...
 -- speed / jump / bhop (solo escribir si cambi el valor)
 local ch = myHum()
...
-- Fly (Heartbeat, compatible mvil con joystick)
RunService.Heartbeat:Connect(function(dt)
 ...
 local cf = camera.CFrame
 local mv = Vector3.zero
 local md = h.MoveDirection -- funciona con joystick mvil
 ...
 -- joystick arriba = avanzar; si mira arriba/abajo tambin vuela vertical un poco
...
task.spawn(function()
 ...
 -- hitbox + wallbang en loop LENTO (0.5s) y solo si cambi: mucho menos detectable
 ...
 -- (El viejo loop de CanCollide no afectaba a las balas y se elimin.)
...
-- Triggerbot: dispara solo cuando un enemigo est bajo la mira (usa el FOV del aimbot).
task.spawn(function()
 local lastShot = 0
 while true do
 if dead then break end
...
if hookmetamethod == nil then
 notify("Executor limitado", "Sin hookmetamethod: Silent y SPY no funcionan aqu. Aimbot, ESP, Trigger, Hitbox y Magic s.")

...
local silentBypass = false
local silentFlash = 0
local silentRedirects = 0
local silentLastTarget = nil
local diagLast, diagF9Last = 0, 0
...
if vel.Magnitude > 60 then vel = vel.Unit * 60 end
pcall(function()
...
if off.Magnitude < 1 or off.Magnitude > 25 then return end
local vel = inst.AssemblyLinearVelocity
if vel.Magnitude < 20 then return end
if vel.Unit:Dot(off.Unit) < 0.2 then return end
local count = 0
...
local aimDeepLast = 0
local lastAimPart = nil
local espFrame, noclipLast, fbLastRefresh = 0, 0, 0
RunService.RenderStepped:Connect(function(dt)
if dead then return end
